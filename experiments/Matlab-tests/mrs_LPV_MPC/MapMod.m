classdef MapMod
    ...
    methods
        %% Constructor:
        function self = MapMod(selectedTrack)
            ...
            roads = size(spec,3); 
            PointAndTangent = zeros(size(spec,1) + 1, 6,roads);
            y_ini = [2*self.halfWidth,2*2*self.halfWidth];

            for k = 1:roads
                for i = 1:size(spec,1)
                    if spec(i, 2, k) == 0.0                % If the current segment is a straight line
                        l = spec(i, 1, k);                 % Length of the segments
                        if i == 1
                            ...
                            y = y_ini(k) + l * sin(ang);             % y coordinate of the last point of the segment
                        else
                            ang = PointAndTangent(i-1, 3,k);                % Angle of the tangent vector at the starting point of the segment
                            x = PointAndTangent(i-1, 1,k) + l * cos(ang);  % x coordinate of the last point of the segment
                            y = PointAndTangent(i-1, 2,k) + l * sin(ang);  % y coordinate of the last point of the segment
                        end
                        
                        psi = ang;  % Angle of the tangent vector at the last point of the segment
    
                        if i == 1
                            NewLine = [x, y, psi, PointAndTangent(i, 4,k), l, 0];
                        else
                            NewLine = [x, y, psi, PointAndTangent(i-1,4,k) + PointAndTangent(i-1,5,k), l, 0];
                        end
                        
                        PointAndTangent(i,:,k) = NewLine;  % Write the new info
                    else
                        l = spec(i,1,k);                 % Length of the segment
                        r = spec(i,2,k);                 % Radius of curvature
                        ...
                        if i == 1
                            ...
                            ang = PointAndTangent(i - 1, 3,k);                              % Angle of the tangent vector at the
                                                                                          % starting point of the segment
                            CenterX = PointAndTangent(i-1, 1, k) ...
                                      + abs(r) * cos(ang + direction * pi / 2);  % x coordinate center of circle
                            CenterY = PointAndTangent(i-1, 2, k) ...
                                      + abs(r) * sin(ang + direction * pi / 2);  % y coordinate center of circle
                        end
                        
                        spanAng = l / abs(r);                    % Angle spanned by the circle
                        psi = wrapToPi(ang + spanAng * sign(r));     % Angle of the tangent vector at the last point of the segment
    
                        angleNormal = wrapToPi((direction * pi / 2 + ang));
                        ...
                        if i == 1
                            NewLine = [x, y, psi, PointAndTangent(i, 4,k), l, 1 / r];
                        else
                            NewLine = [x, y, psi, PointAndTangent(i-1, 4,k) + PointAndTangent(i-1, 5,k), l, 1 / r];
                        end
                ...
                xs = PointAndTangent(end-1, 1, k);
                ys = PointAndTangent(end-1, 2, k);
                xf = 0;
                yf = y_ini(k);
                psif = 0;
                ...
                NewLine = [xf, yf, psif, PointAndTangent(end-1, 4, k) + PointAndTangent(end-1, 5, k), l, 0];
                PointAndTangent(end, :, k) = NewLine;
      
                self.TrackLength(k) = PointAndTangent(end, 4, k) + PointAndTangent(end, 5, k);            
    %             self.TrackLength = PointAndTangent(end, 4); 
        ...
        function [ x, y, theta ] = getGlobalPosition( self, s, ey, lane )
...
%                 theta = angle + direction * spanAng;
        ...
        function [ s, ey, epsi ] = getLocalPosition( self, x, y, psi,lane )
            ...
            for i = 1:size(PointAndTangent,1)
                ...
                if PointAndTangent(i, 6) == 0.0  % If segment is a straight line
                    ...
                    psi_unwrap = wrapToPi(psi); %%%%%%% en un principio era unwrap y otra funcion
                    
                    epsi = psi_unwrap - PointAndTangent(i, 3); % etheta = real - desired 
                    ...
                    elseif norm([xf, yf] - [x, y]) == 0
                        ...
                        if abs(computeAngle( [x,y] , [xs, ys], [xf, yf])) <= pi/2 && abs(computeAngle( [x,y] , [xf, yf], [xs, ys])) <= pi/2
                            ...
                            if abs(ey)<= 4*self.halfWidth + self.slack
                                CompletedFlag = 1;
                            end
                        end
                    end
                    ...
                    elseif norm([xf, yf] - [x, y]) == 0
                        ...
                        if sign(arc1) == sign(arc2) && abs(arc1) >= abs(arc2)
                            ...
                            psi_unwrap = (psi);             %% No estoy seguro de que esto este bien, pero el angulo ahora no nos hace falta.
                            epsi = psi_unwrap - (ang + arc2);

                            if abs(ey) <= 4*self.halfWidth + self.slack % OUT OF TRACK!!
                                CompletedFlag = 1;
                            end
                        end
                    end
