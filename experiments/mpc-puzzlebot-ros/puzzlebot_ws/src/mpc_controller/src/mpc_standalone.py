...
class controller: 
  ...
  def __init__(self):
    ...
    self.xo = np.asarray([0,0,0])[:,np.newaxis]
    self.dt = 0.15
    ...
    xr = self.compute_ref()
    x0 = np.asarray([0.6,1.325,0])
    xmax = np.asarray([np.inf, np.inf, np.inf])
    xmin = np.asarray([-np.inf, -np.inf, -np.inf])
    umax = np.asarray([10, 10])
    umin = np.asarray([-10.0, -10.0])

    self.initi_optimizer(self.Q, self.QN, self.R, self.N, xr, x0, self.nx, self.nu, xmax, xmin, umax, umin)
  ...
  def sim(self,ctrl,x0):
    cmd = np.asarray(ctrl)[:,np.newaxis]

    print(self.Ad*x0)
    print(self.Bd*cmd)

    x = self.Ad*x0 + self.Bd*cmd
    return x
  ...
  def run(self):
    ...
    while not rospy.is_shutdown() and self.it < self.nsim:
      ...
      self.update_optimizer(ref,self.xo[:,0])
