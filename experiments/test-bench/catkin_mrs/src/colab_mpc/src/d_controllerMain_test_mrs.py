...
class agent():
    ...
    #TODO: define Q and R
    def __init__(self, N, Map, dt, Q=None,R=None):

        self.map = Map
        self.N = N
        self.dt = dt
        self.Q  = np.diag([120.0, 1.0, 1.0, 70.0, 0.0, 1500.0])   #[vx ; vy ; psiDot ; e_psi ; s ; e_y]
        self.R  = 0.0000001 * np.diag([3, 0.8])                         #[delta ; a]
        self.Controller = PathFollowingLPV_MPC(Q, R, N, dt, Map, "OSQP")

    def solve(self, x0, agents,lambdas):
        Last_xPredicted = np.array(x0)

        Last_xPredicted, uPred = self.predicted_vectors_generation_V2(self.N, Last_xPredicted, 0.033, map)
        feas, Solution = self.Controller.solve(x0, Last_xPredicted, uPred, False, "A_L", "B_L", "C_L", 4, lambdas, agents)
