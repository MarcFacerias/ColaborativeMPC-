...
class PathFollowingNL_MPC:
    ...
    def eq_constraints(self,states, u):
        # model constants
        lf = self.lf
        lr = self.lr
        m  = self.m
        ...
        for j in range(1, self.N + 1):
            ...
            A12 = (np.sin(u[j, 0]) * Cf) / (m * states[j,0])
            A13 = (np.sin(u[j, 0]) * Cf * lf) / (m * states[j,0]) + states[j,1]

            A22 = -(Cr + Cf * np.cos(u[j, 0])) / (m * states[j,0])
            A23 = -(lf * Cf * np.cos(u[j, 0]) - lr * Cr) / (m *states[j,0]) - states[j,1]

            A32 = -(lf * Cf * np.cos(u[j, 0]) - lr * Cr) / (I * states[j,0])
            A33 = -(lf * lf * Cf * np.cos(u[j, 0]) + lr * lr * Cr) / (I * states[j,0])

            B11 = -(np.sin(u[j, 0]) * Cf) / m
            B12 = 1

            A11 = -mu

            A51 = (1 / (1 - states[j,1] * cur)) * (-cur)
            A52 = (1 / (1 - states[j,0] * cur)) * (np.sin( states[j,4]) * cur)

            A61 = np.cos( states[j,4]) / (1 - states[j,1] * cur)
            A62 = -np.sin( states[j,4]) / (1 - states[j,1] * cur)

            A7 = 1
            A8 = states[j,0]

            A81 = np.cos(states[j,5])
            A82 = -np.sin(states[j,5])

            A91 = np.sin(states[j,5])
            A92 = np.cos(states[j,5])

            B21 = (np.cos(u[j, 0]) * Cf) / m
            B31 = (lf * Cf * np.cos(u[j, 0])) / I
