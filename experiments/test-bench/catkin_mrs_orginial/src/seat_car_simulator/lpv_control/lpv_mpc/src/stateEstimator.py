...
def main():
    ...
    mode        = rospy.get_param("state_estimator/estimatorON")
    ...
    mode             = 1 # TODO: modify this a little bit 
    read_flag        = flag() 
    ...
    while not rospy.is_shutdown():
        ...
        if mode:
            ...
            if sim.vx > 1.0 and sim.vx < 1.4:
                est.estimateState(ecu, sim, est.GS_LPV_Est, est.Continuous_AB_Comp, est.L_Gain_Comp)
            ...
            if sim.vx >= 1.4: 
                ...
                estMsg.v        = np.sqrt(sim.vx**2 + sim.vx**2)
                estMsg.x        = sim.x
                estMsg.y        = sim.y
                estMsg.v_x      = sim.vx
                estMsg.v_y      = sim.vy
                estMsg.psi      = sim.yaw
                estMsg.psiDot   = sim.psiDot
                ...
                est.vx_est      = sim.vx
                est.vy_est      = sim.vy
                est.v_est       = np.sqrt(sim.vx**2 + sim.vx**2)
                est.yaw_est     = sim.yaw
                est.psiDot_est  = sim.psiDot

                est.states_est  = np.array([ est.vx_est, est.vy_est, est.psiDot_est, est.x_est, est.y_est, est.yaw_est ])
    ...
    print "Finishing saving state estimation data"
...
class Estimator(object):
    ...
    def __init__(self,t0,loop_rate,a_delay,df_delay):
        ...
        base = rospack.get_path('lpv_mpc')

        estimator_path              = base + '/src/data/OBSERVER_GAIN/LPV'
        ...
        self.x_est          = 0.02
        self.y_est          = 0.0
        self.vx_est         = rospy.get_param("init_vx")
        self.vy_est         = 0.0
        self.v_est          = 0.0
        self.ax_est         = 0.0
        self.ay_est         = 0.0
    ...
    #
    # command update
    def estimateState(self, ecu, sim, observer, AB_comp, L_comp ):
        """
        ...
        #print("sim", sim.vx, sim.psiDot, sim.x, sim.y, sim.yaw)
    ...
    def GS_LPV_Est(self, states_est, y_meas, u, Continuous_AB_Comp, L_Gain_Comp ):
        ...
        ## Eugenio's fast interesting test:
        # self.vx_est     = y_meas[0]
        # self.vy_est     = 0
        # self.psiDot_est = y_meas[1]
        # self.x_est      = y_meas[2]
        # self.y_est      = y_meas[3]
        # self.yaw_est    = y_meas[4]
    ...
    def Continuous_AB_Comp(self, vx, vy, theta, steer):
        ...
        self.B_obs = np.array([[ B11, np.cos(steer) ], #[delta, a]
                               [ B21, np.sin(steer) ],
                               [ B31,    0.],
                               [ 0.,     0.],
                               [ 0.,     0.],
...
class SimulatorClass(object):
    ...
    def __init__(self,t0):
        ...
        rospy.Subscriber('vehicle_state', VehicleState, self.simulator_callback, queue_size=1)
    ...
    def simulator_callback(self,data):
        ...
        self.x       = data.x
        self.y       = data.y
        self.yaw     = data.heading
        self.vx      = data.longitudinal_velocity
        self.vy      = data.lateral_velocity
        self.psiDot  = data.angular_velocity
...
#             t0: starting measurement time
#         """
#         rospy.Subscriber('ModelPredictiveControlCommand', ModelPredictiveControlCommand, self.ecu_callback, queue_size=1)
...
#         if data.acceleration_command != 0
#             data.motor = -data.brake_deceleration_command

#         else:
#             data.motor = data.acceleration_command 

#         self.df = left_steer_turn_radius_reciprocal_command
