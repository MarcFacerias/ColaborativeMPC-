...
% System definition:
n_meas        = 8; % num. measured states
n_s           = 8; % num. system states
n_sched_vars  = 5; % num. schedulng variables 
...
theta_vec_LM = SchedVars_Limits(7,:)';

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Diseño del observador:

Q   = 10*diag([ 0.05   0.03   0.03  0.25  0.25  0.25 ]);  	% [ vx, vy, wz, x, y, theta ]
R   = 10*diag([ 0.01   0.02   0.02  0.01  0.01 ]);              % measured variables

A_obs       = zeros(6, 6, 2^n_sched_vars);   
C_obs       = zeros(6, 6, 2^n_sched_vars);       
    ...
    for i=1:2
        ...
        for j=1:2
            ...
            for k=1:2
                ...
                for q=1:2
                    Theta_LM = theta_vec_LM(k);
                    [A, B, A_obs(:,:,index), B_obs, C_obs, Ef, Opseudinv ] = Continuous_ABC_Comp_LM( Steer, Vx, Vy, Theta, Theta_LM, 'Invariant B' );
