...
for k=1:TOT_SIM
    ...
    if change_side ~=0 && change_limits_counter == 0
        side = change_side;

        for j = 1:length(XX_dt(1,:))
            [ s(k+j-1), XX_dt(4,j), XX_dt(5,j) ] = map.getLocalPosition( xp(end,j), yp(end,j) ,wrap(yaw(j)), side);
...
%     Update limits dependign on obstacles
    ...
    for i=1:length(XX_dt(1,:))
        yaw(i)  = wrap(tThetaref(i) + XX_dt(5,i)); 
    ...
    if change_limits_counter > 0
        if change_side == 2
            left_limit = (0.5)*ones(1,Hp+1); 
            right_limit = (-0.5 - change_limits_counter*0.1 )*ones(1,Hp+1); % inital track limits +/- 0.5
            change_limits_counter = change_limits_counter -1; 
        elseif change_side ==1
    ...
    [~, ~, detect, s_obs] = Obs_AvoidanceMod(left_limit, right_limit, Obs_Pose1, map, ...
                                                         [xp(k,:);yp(k,:);yaw], Hp, SS_lap, 1, side );
    
    if detect && overtaking_flag == 0 && change_limits_counter == 0  && side ~= 2
        overtaking_flag = 1; 
        change_side = 2;
        start_moving_flag = 1; 
    
    elseif  ~detect && overtaking_flag == 1 && side == 2 && change_limits_counter == 0
        crash = detectFreeLane(Obs_Pose1,  [xp(k,:);yp(k,:);yaw],Hp,1.5 ); 
        if(~crash)
            overtaking_flag = 0;
            change_side = 1;
        end
    ...
    if start_moving_flag
        s_obs = s_obs + v * Tss;
        [x_obs, y_obs, th_obs] = map.getGlobalPosition( s_obs, 0, 1);
        Obs_Pose1(1) = x_obs;
        Obs_Pose1(2) = y_obs;
    end
